...
import { apiClient } from "@/app/utils/api"; // Import hàm gọi API của bạn
...
export default function LoginPage() {
  ...
  const [errorMsg, setErrorMsg] = useState(""); // State chứa thông báo lỗi

  const handleLogin = async (e: React.FormEvent) => {
    e.preventDefault();
    setLoading(true);
    setErrorMsg(""); // Xóa lỗi cũ khi bắt đầu thử đăng nhập lại
    
    try {
      // 1. Gọi API Backend
      const response = await apiClient('/auth/sign-in', {
        method: 'POST',
        body: { email, password }
      });
      ...
      if (response.user.role === 'admin') {
        router.push('/dashboard'); // Là admin thì vào trang quản trị
      } else {
        router.push('/'); // Là user thường thì ra trang chủ
      }

    } catch (error: any) {
      console.error("Login error:", error);
      // Bạn có thể lấy lỗi từ BE nếu api.ts có trả về, hoặc set cứng một câu
      ...
      <div className="flex-grow flex items-center justify-center px-4 py-12">
        <div className="w-full max-w-md">
          ...
          <form onSubmit={handleLogin} className="space-y-6">
            ...
            {/* Khung hiển thị lỗi nếu đăng nhập sai */}
            {errorMsg && (
              <div className="bg-red-50 border border-red-200 text-red-600 text-sm font-medium p-3 rounded-lg text-center">
                {errorMsg}
              </div>
            )}
          ...
          <p className="text-left text-sm mt-6">
            <Link href="/forgot-password" className="text-blue-500 underline hover:text-blue-700 font-medium">
          ...
          <p className="text-left text-xs text-gray-500 mt-4 leading-relaxed">
            By continuing with Google, Apple or Email, you agree to Turskart's{" "}
          ...
          <p className="text-center text-sm text-gray-600 mt-6">
            ...
            <Link href="/signup" className="text-blue-600 underline hover:text-blue-700 font-medium">
              Sign up
            </Link>
          </p>
        </div>
      ...
      <div className="bg-teal-700 text-white py-12 px-4">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <div className="flex gap-4 justify-center flex-wrap">
            <button className="px-8 py-3 bg-white text-[#006557] font-semibold rounded-lg hover:bg-gray-100 transition-colors">
              Post Your Car
            </button>
            <button className="px-8 py-3 border-2 border-white text-white font-semibold rounded-lg hover:bg-teal-600 hover:text-white transition-colors">
              Learn More
            </button>
          </div>
        </div>
